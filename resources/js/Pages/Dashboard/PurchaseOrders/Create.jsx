...
export default function Create({ warehouses = [], products = [], suppliers = [] }) {
    ...
    // Fetch last price for product
    const fetchLastPrice = async (productId, itemId) => {
        try {
            const response = await axios.get(route('purchase-orders.lastPrice'), {
                params: {
            ...
            if (response.data.price) {
                setItems(prev => prev.map(item =>
                    item.id === itemId
                        ? { ...item, unit_price: formatCurrency(response.data.price) }
                        : item
                ))
            }
        } catch (error) {
            console.warn('Failed to fetch last price:', error)
        }
    }
        ...
        setItems(prev => prev.map(item => {
            if (item.id === itemId) {
                const updated = {
                    ...
                    unit_price: option?.buy_price ? formatCurrency(option.buy_price) : '',
                }
                // Fetch last price from supplier
                if (option?.value && data.supplier_id) {
                    fetchLastPrice(option.value, itemId)
                }
                return updated
            }
            return item
    ...
    const submit = (e) => {
        ...
        if (hasDuplicates) {
            toast.error('Terdapat produk yang duplikat. Silakan hapus atau gabungkan item yang sama.')
            return
        }
            ...
            <form onSubmit={submit}>
                <div className='grid grid-cols-1 lg:grid-cols-3 gap-4'>
                    ...
                    <div className='lg:col-span-2 space-y-4'>
                        {/* Header Info */}
                        <Card title='Informasi PO' className='mb-4'>
                                ...
                                <table className='w-full text-sm'>
                                    <thead>
                                        <tr className='border-b dark:border-gray-700'>
                                            <th className='text-left py-2 px-2 font-medium text-gray-700 dark:text-gray-300' style={{ minWidth: '250px' }}>Produk</th>
                                            <th className='text-center py-2 px-2 font-medium text-gray-700 dark:text-gray-300' style={{ width: '100px' }}>Qty</th>
                                            <th className='text-right py-2 px-2 font-medium text-gray-700 dark:text-gray-300' style={{ width: '150px' }}>Harga Satuan</th>
                                            <th className='text-right py-2 px-2 font-medium text-gray-700 dark:text-gray-300' style={{ width: '150px' }}>Subtotal</th>
                                            <th className='text-center py-2 px-2 font-medium text-gray-700 dark:text-gray-300' style={{ width: '50px' }}></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                            ...
                                            <tr key={item.id} className='border-b dark:border-gray-700'>
                                                ...
                                                <td className='py-2 px-2'>
                                                    <Input
                                                        ...
                                                        onChange={(e) => handleItemChange(item.id, 'unit_price', formatCurrency(e.target.value))}
                                                        placeholder='0'
                        ...
                        <Card title='Ringkasan' className='sticky top-4'>
                            ...
                            <div className='mt-6'>
                                <Button
                                    ...
                                    label={processing ? 'Menyimpan...' : 'Simpan PO (Draft)'}
                                />
                            </div>
                        </Card>
                    </div>
